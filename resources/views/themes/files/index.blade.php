<x-app-layout>
    <div class="w-full h-full p-4">
        ...
        <div class="mb-4">
            <form action="{{ route('file.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    ...
                    @foreach ($filesList as $file)
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('file.edit', $file['name']) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">
                                    Edit
                                </a>
                                @if ($file['in_trash'])
                                    <a href="#!" onclick="openRestoreModal()" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 ml-4">
                                        Restore
                                    </a>
                                @else
                                    <form action="{{ route('file.delete', $file['name']) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                            @foreach ($trashFiles as $file)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <form action="{{ route('file.restore', $file) }}" method="GET" style="display:inline-block;">
                                            @csrf
